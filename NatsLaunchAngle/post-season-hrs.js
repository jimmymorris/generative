...
const sketch = async () => {
  const statcast = await load('./nats-postseason-hr.json');
  const GRAVITY = 9.8;
  ...
  return ({ p5, context, width, height }) => {
    ...
    for (let i = 0; i < batOnBall.length; i += 1) {
      ...
      let RBI = 2;
      if (batOnBall[i].on_1b !== "null") {
        RBI += 3
      }
      if (batOnBall[i].on_2b !== "null") {
        RBI += 4
      }
      if (batOnBall[i].on_3b !== "null") {
        RBI += 5
      }

      teams.add(batOnBall[i].home_team);
      teams.add(batOnBall[i].away_team);
